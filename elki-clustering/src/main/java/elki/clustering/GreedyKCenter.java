...
public class GreedyKCenter<O> implements ClusteringAlgorithm<Clustering<SimplePrototypeModel<O>>> {
  ...
  public Clustering<SimplePrototypeModel<O>> run(Relation<O> relation) {
    ...
    SimplePrototypeModel<O>[] heads = new SimplePrototypeModel[k];

    DBIDVar curHead = DBIDUtil.newVar();

    int headID = 0; // could be random as well
    curHead = aids.assignVar(headID, curHead);
    heads[0] = new SimplePrototypeModel<O>(relation.get(curHead));
    ...
    for(int i = 1; i < k; i++) {
      ...
      heads[i] = new SimplePrototypeModel<O>(relation.get(curHead));
    ...
    for(int i = 0; i < clusters.length; i++) {
      result.addToplevelCluster(new Cluster<SimplePrototypeModel<O>>(clusters[i], heads[i]));
    }
    return result;
  }
